...
def write_submission(df, top_k, path):
    pred_dict = defaultdict(list)
    for row in df.itertuples(index=False):
        heapq.heappush(pred_dict[row.query_id], (row.product_id, row.score))
    result = {
        query_id: list(zip(*heapq.nlargest(top_k, product_id_score_tuple, key=lambda x: x[1])))[0]
        for query_id, product_id_score_tuple in pred_dict.items()
    }
    with open(path, 'w') as f:
        f.write('query-id,product1,product2,product3,product4,product5' + '\n')
        for query_id, product1_id_tuple in result.items():
            f.write(
                ','.join([str(query_id)] + list(map(str, product1_id_tuple))) + '\n'
            )
